...
const renderGame = (el) => {
  ...
  <div class="game_wrapper">
    <div class="cover" onClick="appDetail(${el["appId"]})">
      ...
      src="${
        el["imgUrl"]
          ? el["imgUrl"].replace(/\/\w+.jpg/, "/header.jpg")
          : "no-image.png"
      }"
      onerror="this.onerror=null;this.src='no-image.png';" data-id="${
        el["appId"]
      }"
      />`
          : `<div class="no-image-cover" data-id="${el["appId"]}"></div>`
      }
      <div class="game_info">
        <h3 class='game-title'>${el["title"]}</h3>
        <p>${el["price"]}</p>
      </div>
    </div>
  </div>`;
